import pygame
from Solver import solve, isValid, find_empty
import time
pygame.font.init()
...
class Grid:
    ...
    def __init__(self, rows, cols, width, height):
        self.rows = rows
        self.cols = cols
        self.cubes = [[Cube(self.board[i][j], i, j, width, height) for j in range
    ...
    def auto_solve(self):
        position = find_empty(self.cubes)
        if not position:
            return True
        else:
            row, col = position
        for i in range(1, len(self.model) + 1):
            if isValid(self.model, i, position):
